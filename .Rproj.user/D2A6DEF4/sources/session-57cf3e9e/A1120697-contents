...
test_that("bio_07 computes temperature annual range correctly", {
  ...
  # Expected: max(1:12) - min(1:12) = 12 - 1 = 11
...
test_that("bio_07 fails with wrong class inputs", {
  ...
  expect_error(bio_07(wrong_input, tasmax), "Must inherit from class 'SpatRaster'")
  expect_error(bio_07(tasmax, wrong_input), "Must inherit from class 'SpatRaster'")
})

test_that("bio_07 fails with invalid filename", {
  tasmax <- mock_tas()
  tasmin <- mock_tas()
  expect_error(bio_07(tasmax, tasmin, filename = 123), "Must be of type 'string'")
})