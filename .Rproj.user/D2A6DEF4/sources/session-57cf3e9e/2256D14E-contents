#' Row-wise maximum (parallel)
#'
#' Fast row-wise **maximum value** using a parallel C++ implementation via
#' \pkg{RcppParallel}. This is tailored for large matrices derived from rasters
#' (rows = grid cells, columns = months), which is a common layout when working
#' with CHELSA monthly climatologies.
#'
#' @details
#' Typical CHELSA Bioclim use cases include computing:
#' - **BIO5** (Max Temperature of Warmest Month): the per-cell maximum across
#'   the 12 monthly temperature layers of the relevant CHELSA temperature
#'   product.
#' - Analogous operations for other variables where the month-wise maximum is
...
#'
#' @return A **numeric vector** of length `nrow(mat)` with the row-wise maximum
...
#' )
#' # Rows are: c(1,4,7), c(2,5,8), c(3,6,9) -> maxima: 7, 8, 9
#' mx <- parallel_row_max(mat)
#' mx
#'
#' # CHELSA-style usage:
#' tas <- mock_tas()
...
#' warmest <- parallel_row_max(tas_matrix)
#' all(warmest == 12)
#' @seealso [parallel_row_min()], [parallel_sd()], [parallel_variance()]
#' @export
parallel_row_max <- function(mat) {
  ...
  res_mat <- rcpp_parallel_which_max_row(mat)
