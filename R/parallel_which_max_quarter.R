...
#' - If `na_rm = FALSE` (default), any `NA` in a window **invalidates** that
#'   window.
#' - If `na_rm = TRUE`, the score is computed as the
#'   **mean over non-NA values** windows with **all NA** are invalid.
#'   If **no valid windows** exist for a row, the result is `NA`.
#'
#' @param mat Numeric matrix (rows = cells, columns = months). Must have
#'   at least 3 columns.
#' @param wrap Logical; if `TRUE`, allow wrap-around windows
#'   (e.g., Dec–Jan–Feb). If `FALSE`, only windows
#'   `[1..3], [2..4], ..., [10..12]` are considered. This is particularly needed
#'   when calculate bioclimatic variables by an specific year
