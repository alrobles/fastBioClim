...
#' indices provided in `idx` (1-based).
#'
...
#' (e.g., Dec–Jan–Feb).
#' @param na_rm Logical; if `TRUE`, ignore `NA`s in the 3-month average
#'   (return `NA` only if all three are `NA`). If `FALSE`, any `NA` in the
#'   window propagates to `NA`.
#'
#' @return A numeric vector of length `nrow(mat)` with the per-row averages.
#' @export
parallel_average_quarter <- function(idx, mat, wrap = FALSE, na_rm = FALSE) {
  ...
  res <- rcpp_parallel_average_quarter(idx, mat, wrap = wrap, na_rm = na_rm)
