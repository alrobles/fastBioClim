...
#' \code{parallel_which_max_rolling_quarter(pr_matrix, wrap = ...)}, and the
#' quarter mean temperature is computed with
#' \code{parallel_average_quarter_from_index(idx, tas_matrix)} which averages
...
#'
#' @seealso
#' [parallel_which_max_rolling_quarter()], [parallel_average_quarter_from_index()],
bio_08 <- function(pr, tas, wrap = FALSE, filename = ""){
  ...
  for (i in 1:b$n) {
    ...
    r_max_tasmax <- parallel_which_max_rolling_quarter(v_pr, wrap = wrap)
    ...
    r <- parallel_average_quarter_from_index(r_max_tasmax, v_tas)
