...
#'
#' Compute **BIO11 (Mean Temperature of Coldest Quarter)** from monthly mean
#' temperature (`tas`) rasters following the CHELSA/WorldClim Bioclimatic
#' Variables framework.
#'
#' @details
#' **BIO11** is the **average monthly mean temperature over the coldest
#' 3‑month period** of the year, computed per cell. This implementation:
#' \enumerate{
#'   \item Identifies, for each cell, the starting month of the **coldest
#'         quarter** by scanning rolling 3‑month windows over \code{tas}
#'         (optionally allowing wrap‑around windows, e.g., Dec–Jan–Feb when
#'         \code{wrap = TRUE}) and selecting the window with the **minimum
#'         score** (see NA handling below).
...
#' \code{parallel_which_min_quarter(tas_matrix, wrap = ..., na_rm = ...)}.
...
#'   are ignored when computing window scores (for selection) and the final
#'   quarter mean. A window with **all \code{NA}** values yields \code{NA};
#'   if **no valid windows** exist for a row/cell, the result is \code{NA}.
#' - If \code{na_rm = FALSE} (default), any \code{NA} in a 3‑month window
#'   **invalidates** that window for selection; the final quarter mean will also
#'   become \code{NA} if the selected window contains any \code{NA}.
#'
...
#'
#' @return A [terra::SpatRaster] with one layer representing **BIO11**
#'   (Mean Temperature of Coldest Quarter, units °C).
#'
#' @references
...
#'
#' # Compute BIO11 without wrap-around (only Jan–Dec contiguous quarters)
#' bio11 <- bio_11(tas_ex, wrap = FALSE)
#' bio11
#'
#' # Allow wrap-around quarters (e.g., Dec–Jan–Feb)
#' bio11_wrap <- bio_11(tas_ex, wrap = TRUE)
#' bio11_wrap
#'
#' # Handle NA values by ignoring them during selection and averaging
#' tas_na <- make_1x1_12(c(NA, NA, -5, -3, -2, -1, 0, 1, 2, 3, 4, 5))
#' bio11_na <- bio_11(tas_na, wrap = FALSE, na_rm = TRUE)
#' bio11_na
#'
#' @export
bio_11 <- function(tas, wrap = FALSE, filename = "", na_rm = FALSE) {
  ...
  for (i in 1:b$n) {
    ...
    r_min_tas <- parallel_which_min_quarter(v_tas, wrap = wrap, na_rm = na_rm)

    r <- parallel_average_quarter(
      idx = as.matrix(r_min_tas),
      mat = v_tas,
      wrap = wrap,
      na_rm = na_rm
    )
